...
var bro = {};
    ...
    bro.host = window.location.host;
    
    // 设置 domain
    // document.domain = 'dreamstu.com';
    ...
    //browser.out(browser.name);
    //browser.out(browser.ua);
    ...
    */
    ...
    var adjustBehaviors = function() {
        ...
        if (browser.ie && browser.ie < 7) {
            try {
                ...
                //browser.out("错误：[" + e.name + "] "+e.message+", " + e.fileName+", 行号:"+e.lineNumber+"; stack:"+typeof e.stack, 2);
            }
        }
    }
    
    if(bro.browserOptions.adjustBehaviors){
         adjustBehaviors();
    }
    
    var filterDot = function(string){
        //return browser.string.replaceAll(string, "\.", "_");
    ...
    if(bro.browserOptions.htmlClass){
        addHtmlClassName();
    }
